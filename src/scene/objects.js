...
export async function loadWorldObjects(scene) {
  ...
  // Load fence
  const fenceModel = await fbxloader.loadAsync('resources/world_objects/fence.fbx');
  gameState.objects.fence = fenceModel.scene ? fenceModel.scene : fenceModel;
  scene.add(gameState.objects.fence);

  // Load house
  const houseModel = await gltfloader.loadAsync('resources/world_objects/cottage.glb');
